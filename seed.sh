...
mkdir -p arena/seed/src/{musl,make,busybox,linux}
...
tar -C arena/seed/src/musl --strip-components=1 -xzf \
...
pushd arena/stage/1/obj
	mkdir -p musl/crt
	mkdir -p musl/src/{env,errno,exit,internal,locale}
	mkdir -p musl/src/{math,multibyte}
	mkdir -p musl/src/{signal,stdio,string,thread,unistd}
popd


# Code host-processing hacks and workarounds

pushd arena/seed/src/musl
