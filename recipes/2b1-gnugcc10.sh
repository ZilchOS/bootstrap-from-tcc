...
mkdir -p /tmp/2b1-gnugcc10; cd /tmp/2b1-gnugcc10
...
export PATH="/tmp/2b1-gnugcc10/aliases:$PATH"
...
echo '#!/store/1-stage1/protobusybox/bin/ash' > wrappers/c++
...
echo 'exec g++ $_SYSROOT -Wl,$_LDFLAG "$@"' >> wrappers/c++
echo 'exec /store/2a5-gnugcc10/bin/cpp $_NEWINC "$@"' >> wrappers/cpp
echo 'exec /store/2a1-static-binutils/bin/ld $_LDFLAG "$@"' >> wrappers/ld
chmod +x wrappers/cc wrappers/c++ wrappers/cpp wrappers/ld
...
ash configure \
	...
	CC=/tmp/2b1-gnugcc10/wrappers/cc \
	CXX=/tmp/2b1-gnugcc10/wrappers/c++ \
	CPP=/tmp/2b1-gnugcc10/wrappers/cpp \
	LD=/tmp/2b1-gnugcc10/wrappers/ld \
	--with-sysroot=$SYSROOT \
	--with-native-system-header-dir=/include \
	--with-build-time-tools=/store/2a1-static-binutils/bin \
	--prefix=/store/2b1-gnugcc10 \
	--with-specs='%{!static:%x{-rpath=/store/2b1-gnugcc10/lib}}' \
